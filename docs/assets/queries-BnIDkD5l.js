import{m as e,v as t}from"./src-BHZJQV51.js";import{t as n}from"./preload-helper-Czpn1I53.js";import{S as r,b as i,ct as a,lt as o,mt as s,st as c,t as l,y as u}from"./handleFs-DCeMKRCB.js";var d=[{version:1,name:`init`,sql:`
-- ── Library ────────────────────────────────────────────────────────────────
...
`}];async function f(e){await e.exec(`
  ...
  )`}async function I(){let e=await O.db.exec(`
    SELECT f.id, f.parent_id, f.name, f.path, f.home,
      (SELECT COUNT(*) FROM toppings t
        WHERE t.folder_id = f.id
          AND t.deleted_at IS NULL
          AND ${F(`t`)}) AS count
    FROM folders f`),t=new Map(e.map(e=>[e.id,{id:e.id,parentId:e.parent_id,name:e.name,count:e.count,vaultPath:we(e),children:[]}])),n=[];for(let e of t.values()){let r=e.parentId?t.get(e.parentId):void 0;r?r.children.push(e):n.push(e)}let r=e=>{e.sort((e,t)=>e.name.localeCompare(t.name)),e.forEach(e=>r(e.children))};return r(n),n}function je(e){let t=new Map;for(let n of e)!n.thumb_ref||t.has(n.folder_id)||t.set(n.folder_id,{thumbRef:n.thumb_ref,thumbColor:n.thumb_color,...n.updated_at?{updatedAt:n.updated_at}:{}});return t}async function Me(e){let t=[...new Set(e)];if(t.length===0)return new Map;let n=t.map(()=>`?`).join(`,`);return je(await O.db.exec(`WITH ranked AS (
     ...
     SELECT folder_id, thumb_ref, thumb_color, updated_at
       FROM ranked
      WHERE recency_rank = 1`,t))}var L={key:`$updated`,dir:`desc`},Ne={sorts:[L],filters:null,groupBy:null};function Pe({total:e,thumbs:t,docs:n}){return e<4||t/e>=.5?`masonry`:n/e>=.6?`list`:`masonry`}function R(e){if(!Array.isArray(e))return;let n=new Set,r=[];for(let i of e){let e=typeof i==`string`?i:i&&typeof i==`object`&&typeof i.key==`string`?i.key:``;if(!e||e.startsWith(`$`)||n.has(e))continue;n.add(e);let a=i&&typeof i==`object`?t(i.width):160;r.push({key:e,width:a})}return r}function Fe(e){if(!e||typeof e!=`object`||Array.isArray(e))return;let t={};for(let[n,r]of Object.entries(e))n&&typeof r==`string`&&r.trim()&&(t[n]=r.trim());return Object.keys(t).length>0?t:void 0}function z(e){if(e===`title`)return[{key:`$title`,dir:`asc`}];if(e===`updated`)return[L];let t=(Array.isArray(e)?e:[e]).flatMap(e=>{if(!e||typeof e!=`object`||Array.isArray(e))return[];let t=e;return typeof t.key!=`string`||!t.key?[]:[{key:t.key,dir:t.dir===`asc`?`asc`:`desc`}]});if(t.find(e=>e.key===`$manual`))return[{key:`$manual`,dir:`asc`}];let n=new Set,r=t.filter(e=>n.has(e.key)?!1:(n.add(e.key),!0));return r.length>0?r:[L]}function B(e){if(typeof e==`string`)return e?{key:e,dir:`asc`}:null;if(!e||typeof e!=`object`)return null;let t=e;return typeof t.key!=`string`||!t.key?null:{key:t.key,dir:t.dir===`desc`?`desc`:`asc`}}function Ie(e,t){try{let n=JSON.parse(e);return JSON.stringify({layout:n.layout,sorts:z(`sorts`in n?n.sorts:n.sort),filters:n.filters,groupBy:`groupBy`in n?B(n.groupBy):t,columns:Array.isArray(n.columns)?R(n.columns)??null:n.columns??null})}catch{return e}}function Le(e){let t=JSON.parse(e),n=z(t.sorts??t.sort);t.colSort&&(n=z(t.colSort));let r=B(t.groupBy),i={sorts:n,filters:t.filters??null,groupBy:r},a=R(t.columns);a&&(i.columns=a);let o=Fe(t.propertyLabels);if(o&&(i.propertyLabels=o),t.roles&&typeof t.roles==`object`&&!Array.isArray(t.roles)){let e={};for(let[n,r]of Object.entries(t.roles))typeof r==`string`&&r&&(e[n]=r);Object.keys(e).length>0&&(i.roles=e)}if(Array.isArray(t.hidden)){let e=t.hidden.filter(e=>typeof e==`string`&&e!==``);e.length>0&&(i.hidden=e)}return t.origin&&(i.origin={...t.origin,spec:Ie(t.origin.spec,r)}),i}async function V(e,t){let n=await e.exec(`SELECT id, name, layout, config, is_default, position FROM views WHERE folder_id IS ? ORDER BY position, name`,[t]);if(n.length===0){let n=`masonry`;if(t!==null){let r=(await e.exec(`SELECT COUNT(*) AS total,
          ...
          WHERE t.folder_id = ? AND t.deleted_at IS NULL
            AND ${F(`t`)}`,[t]))[0];n=Pe({total:r?.total??0,thumbs:r?.thumbs??0,docs:r?.docs??0})}let r={id:`v_${t??`root`}`,name:`Default`,layout:n,isDefault:!0,position:1,cfg:Ne};return await e.exec(`INSERT OR IGNORE INTO views (id, folder_id, name, layout, config, kind, is_default, position) VALUES (?,?,?,?,?,'shared',1,1)`,[r.id,t,r.name,r.layout,JSON.stringify(r.cfg)]),[r]}let r=n.map(e=>({id:e.id,name:e.name,layout:e.layout,isDefault:e.is_default===1,position:e.position,cfg:Le(e.config)}));return r.some(e=>e.isDefault)||(r[0].isDefault=!0),r}function Re(e){return V(O.db,e)}async function H(e,t,n){let r=await e.exec(`SELECT MAX(position) AS maxpos FROM views WHERE folder_id IS ?`,[t]),i={id:`v_${crypto.randomUUID()}`,name:n.name,layout:n.layout,isDefault:!1,position:(r[0]?.maxpos??0)+1,cfg:n.cfg};return await e.exec(`INSERT INTO views (id, folder_id, name, layout, config, kind, is_default, position) VALUES (?,?,?,?,?,'shared',0,?)`,[i.id,t,i.name,i.layout,JSON.stringify(i.cfg),i.position]),i}function ze(e,t){return H(O.db,e,t)}async function Be(e,t){return O.db.transaction(async n=>{let r=await n.exec(`SELECT MAX(position) AS maxpos FROM views WHERE folder_id IS ?`,[e]),i=await n.exec(`SELECT name FROM views WHERE folder_id IS ?`,[e]),a=new Set(i.map(e=>e.name.trim().toLowerCase())),o=[],s=r[0]?.maxpos??0;for(let r of t){let t=r.name.trim().toLowerCase();if(!t||a.has(t))continue;a.add(t);let i={id:`v_${crypto.randomUUID()}`,name:r.name,layout:r.layout,isDefault:!1,position:++s,cfg:r.cfg};await n.exec(`INSERT INTO views (id, folder_id, name, layout, config, kind, is_default, position) VALUES (?,?,?,?,?,'shared',0,?)`,[i.id,e,i.name,i.layout,JSON.stringify(i.cfg),i.position]),o.push(i)}return o})}async function Ve(e,t){await O.db.exec(`UPDATE views SET name = ? WHERE id = ?`,[t,e])}async function U(e,t){await e.exec(`DELETE FROM view_order WHERE view_id = ?`,[t]),await e.exec(`DELETE FROM views WHERE id = ?`,[t])}function He(e){return U(O.db,e)}async function Ue(e,t){await O.db.exec(`UPDATE views SET is_default = 0 WHERE folder_id IS ?`,[e]),await O.db.exec(`UPDATE views SET is_default = 1 WHERE id = ?`,[t])}async function W(e,t,n,r){await e.exec(`UPDATE views SET layout = ?, config = ? WHERE id = ?`,[n,JSON.stringify(r),t])}function We(e,t,n){return W(O.db,e,t,n)}async function Ge(e,t,n){await O.db.exec(`INSERT INTO view_order (view_id, topping_id, order_key) VALUES (?,?,?)
       ON CONFLICT(view_id, topping_id) DO UPDATE SET order_key = excluded.order_key`,[e,t,n])}async function Ke(e,t){let{spacedOrderKeys:r}=await n(async()=>{let{spacedOrderKeys:e}=await import(`./orderKeys-EFJGhdo7.js`);return{spacedOrderKeys:e}},[]),i=r(t.length),a=new Map;return await O.db.transaction(async n=>{for(let r=0;r<t.length;r+=1){let o=t[r],s=i[r];a.set(o,s),await n.exec(`INSERT INTO view_order (view_id, topping_id, order_key) VALUES (?,?,?)
           ON CONFLICT(view_id, topping_id) DO UPDATE SET order_key = excluded.order_key`,[e,o,s])}}),a}async function qe(e,t,n){let r=await e.exec(`SELECT MAX(position) AS maxpos FROM views WHERE folder_id IS ?`,[n]);await e.exec(`UPDATE views SET folder_id = ?, is_default = 0, position = ? WHERE id = ?`,[n,(r[0]?.maxpos??0)+1,t])}function Je(e){return{list:t=>V(e,t),create:(t,n)=>H(e,t,n),delete:t=>U(e,t),moveToFolder:(t,n)=>qe(e,t,n),saveState:(t,n,r)=>W(e,t,n,r)}}var G={eq:`=`,ne:`!=`,lt:`<`,lte:`<=`,gt:`>`,gte:`>=`},K=e=>e.replace(/[\\%_]/g,`\\$&`);function q(e,t){if(e.op!==`cmp`){if(e.children.length===0)return`1`;let n=e.op===`not`?` OR `:` ${e.op.toUpperCase()} `,r=`(`+e.children.map(e=>q(e,t)).join(n)+`)`;return e.op===`not`?`NOT ${r}`:r}if(e.key===`$title`||e.key===`$basename`)return t.push(String(e.value)),e.cmp===`contains`?`INSTR(t.title, ?) > 0`:`t.title ${G[e.cmp]??`=`} ?`;if(e.key===`$name`){let n=String(e.value);t.push(n,`%/${K(n)}`);let r=`(t.content_ref = ? OR t.content_ref LIKE ? ESCAPE '\\')`;return e.cmp===`ne`?`NOT ${r}`:r}if(e.key===`$path`)return t.push(String(e.value)),e.cmp===`contains`?`INSTR(t.content_ref, ?) > 0`:`t.content_ref ${G[e.cmp]??`=`} ?`;if(e.key===`$folder`){let n=`CASE WHEN f.path = '/' THEN '' ELSE SUBSTR(f.path, 2) END`;if(e.cmp===`inFolder`){let r=String(e.value).replace(/^\/+|\/+$/g,``);return t.push(r,`${K(r)}/%`),`(${n} = ? OR ${n} LIKE ? ESCAPE '\\')`}return e.cmp===`startsWith`?(t.push(String(e.value)),`INSTR(${n}, ?) = 1`):(t.push(String(e.value)),e.cmp===`contains`?`INSTR(${n}, ?) > 0`:`${n} ${G[e.cmp]??`=`} ?`)}if(e.key===`$ext`){let n=`%.${K(String(e.value).replace(/^[.]/,``).toLowerCase())}`;return t.push(n),e.cmp===`ne`?`LOWER(t.content_ref) NOT LIKE ? ESCAPE '\\'`:`LOWER(t.content_ref) LIKE ? ESCAPE '\\'`}if(e.key===`$updated`)return t.push(Number(e.value)),`(unixepoch(t.updated_at) * 1000) ${G[e.cmp]??`=`} ?`;if(e.key===`$type`)return t.push(String(e.value)),`t.type = ?`;if(e.key===`$interaction.status`)return t.push(String(e.value)),e.cmp===`ne`?`EXISTS (
        SELECT 1
      ...
      FROM private_entity_effective_marks i
      WHERE i.topping_id = t.id AND i.rating IS NOT NULL
        AND i.rating ${G[e.cmp]??`=`} ?
    )`;if(e.cmp===`tagged`){let n=String(e.value).replace(/^#/,``).toLowerCase();return t.push(n,`${K(n)}/%`),`EXISTS (
      SELECT 1 FROM topping_tags tt JOIN tags g ON g.id = tt.tag_id
      WHERE tt.topping_id = t.id AND (g.name = ? OR g.name LIKE ? ESCAPE '\\')
    )`}return typeof e.value==`number`||typeof e.value==`boolean`?(t.push(e.key,typeof e.value==`boolean`?+!!e.value:e.value),`EXISTS (SELECT 1 FROM properties p WHERE p.topping_id = t.id AND p.key = ? AND p.value_num ${G[e.cmp]??`=`} ?)`):e.cmp===`contains`?(t.push(e.key,String(e.value),String(e.value)),`EXISTS (
    ...
    )`):(t.push(e.key,String(e.value)),`EXISTS (SELECT 1 FROM properties p WHERE p.topping_id = t.id AND p.key = ? AND p.value_text ${G[e.cmp]??`=`} ?)`)}function J(e){if(!e||e.op===`not`||e.op===`or`)return null;if(e.op===`cmp`)return e.key===`$folder`&&(e.cmp===`inFolder`||e.cmp===`eq`)?String(e.value):null;for(let t of e.children){let e=J(t);if(e!==null)return e}return null}function Y(e){return!e||e.op===`not`||e.op===`or`?!1:e.op===`cmp`?e.key===`$folder`&&(e.cmp===`inFolder`||e.cmp===`startsWith`):e.children.some(Y)}function Ye(e,t){return e===null||Y(t)}async function Xe(e,t,n){let r=[],i=``,a,o=t.sorts.length>0?t.sorts:[L],s=o[0]?.key===`$manual`;if(s)i=`LEFT JOIN view_order vo ON vo.topping_id = t.id AND vo.view_id = ?`,r.push(n??``),a=`(vo.order_key IS NULL) ASC, vo.order_key ASC, t.updated_at DESC`;else{let e=[],t=[];for(let[n,i]of o.entries()){let a=i.dir===`asc`?`ASC`:`DESC`;if(i.key===`$updated`)t.push(`t.updated_at ${a}`);else if(i.key===`$created`)t.push(`t.created_at ${a}`);else if(i.key===`$title`||i.key===`$basename`||i.key===`$name`)t.push(`t.title COLLATE NOCASE ${a}`);else if(i.key===`$path`)t.push(`t.content_ref COLLATE NOCASE ${a}`);else if(i.key===`$folder`)t.push(`f.path COLLATE NOCASE ${a}`);else{let o=`s${n}`;e.push(`LEFT JOIN properties ${o} ON ${o}.topping_id = t.id AND ${o}.key = ?`),r.push(i.key),t.push(`(${o}.topping_id IS NULL) ASC`,`${o}.value_num ${a}`,`${o}.value_text COLLATE NOCASE ${a}`)}}i=e.join(` `),a=[...t,`t.id ASC`].join(`, `)}let c=``;e&&!Y(t.filters)&&(c+=` AND t.folder_id = ?`,r.push(e)),t.filters&&(c+=` AND ${q(t.filters,r)}`);let l=(await O.db.exec(`SELECT t.id, t.type, t.title, t.content_ref, t.source, f.name AS folder, t.updated_at, t.thumb_ref, t.thumb_color, t.thumb_aspect${s?`, vo.order_key`:``}
     FROM toppings t JOIN folders f ON f.id = t.folder_id ${i}
     WHERE t.deleted_at IS NULL AND ${F(`t`)} ${c}
     ORDER BY ${a}`,r)).map(e=>({id:e.id,type:e.type,title:e.title,subtitle:e.folder,contentRef:e.source===`vault`?e.content_ref:null,updatedAt:e.updated_at,thumbRef:e.thumb_ref,thumbColor:e.thumb_color,aspect:e.thumb_aspect,...s?{orderKey:e.order_key??null}:{}})),u=await rt(l.map(e=>e.id));for(let e of l){let t=u.get(e.id);t&&(e.interactionMarks=t)}return await Ze(l),l}async function X(){let[e,t]=await Promise.all([O.db.exec(`SELECT id, name, labels FROM status_sets`),O.db.exec(`SELECT match_value, set_id FROM status_set_bindings
        WHERE match_kind = 'schema_type'
        ORDER BY match_value, set_id`)]);return{sets:new Map(e.map(e=>[e.id,{id:e.id,name:e.name,labels:it(e.labels)}])),bindings:t.map(e=>({schemaType:e.match_value,setId:e.set_id}))}}async function Ze(e){let t=e.filter(e=>e.type===`link`);if(t.length===0)return;let[n,r]=await Promise.all([tt(t.map(e=>e.id)),X()]);for(let e of t){let t=e.interactionMarks?.[0]?.setId??null,a=t?null:n.get(e.id),o=r.sets.get(i({pinnedSetId:t,schemaType:a?u(a)?.type??null:null,bindings:r.bindings}));o&&(e.statusSet=o)}}async function Qe(e,t){let n=Q(e),[i,a]=await Promise.all([n===null?Promise.resolve([]):O.db.exec(`SELECT t.folder_id, p.value_text
       ...
       WHERE p.key = 'url' AND t.type = 'link' AND t.deleted_at IS NULL`),O.db.exec(`SELECT folder_id FROM toppings
        ...
        LIMIT 50`)]),o=e===null?null:u(e),s=new Map,c=new Map;for(let e of i)e.value_text&&(Q(e.value_text)===n&&s.set(e.folder_id,(s.get(e.folder_id)??0)+1),o&&u(e.value_text)?.type===o.type&&c.set(e.folder_id,(c.get(e.folder_id)??0)+1));let l=[];for(let e of a)l.includes(e.folder_id)||l.push(e.folder_id);return{...o?{type:{value:o.type,label:r(o.type),tallies:Z(c)}}:{},...n?{domain:{value:n,label:n,tallies:Z(s)}}:{},recent:l,currentFolderId:t}}function Z(e){return[...e].map(([e,t])=>({folderId:e,count:t}))}function Q(e){if(!e)return null;try{let t=new URL(e);return t.protocol!==`http:`&&t.protocol!==`https:`?null:t.hostname.toLowerCase().replace(/^www\./,``)||null}catch{return null}}async function $e(){let[e,t,n,r]=await Promise.all([O.db.exec(`SELECT t.id, t.type, t.title, t.folder_id, f.name AS folder_name,
              t.content_ref
         FROM toppings t
        ...
        WHERE t.source = 'vault'
          AND t.type = 'note'
          AND t.deleted_at IS NULL
        ORDER BY tt.topping_id, tags.name`),I()]),i=new Map,a=e=>{e.vaultPath!==null&&i.set(e.id,{name:e.vaultPath===``?`Vault`:e.vaultPath,path:e.vaultPath}),e.children.forEach(a)};return r.forEach(a),De(e,t,n,i)}async function et(e=new Set){let t=(await O.db.exec(`SELECT COUNT(*) AS total,
      ...
      WHERE t.source = 'vault'
        ...
        AND ${F(`t`)}`,[p,m,1]))[0];if(!t||t.total===0||t.indexed!==t.total)return[];let n=await I(),r=new Map,i=0,a=e=>{e.vaultPath!==null&&(r.set(e.id,{name:e.vaultPath===``?`Vault`:e.vaultPath,path:e.vaultPath}),e.count>=3&&(i+=1)),e.children.forEach(a)};if(n.forEach(a),i===0||t.total*i>5e5)return[];let o=await O.db.exec(`SELECT t.id, t.title, t.folder_id, folder.name AS folder_name,
        ...
        ORDER BY t.id, embedding.segment_kind,
                 embedding.anchor_page, embedding.ordinal`,[p,m,1,384]),s=new Map;for(let e of o){let t=s.get(e.id);t||(t={id:e.id,title:e.title,folderId:e.folder_id,folderName:e.folder_name,path:e.content_ref,vectors:[]},s.set(e.id,t)),t.vectors.push(y(e.vector))}return s.size===t.total?Ae([...s.values()],r,e):[]}async function tt(e){if(e.length===0)return new Map;let t=e.length<=900,n=await O.db.exec(`SELECT p.topping_id, p.value_text
     FROM properties p JOIN toppings t ON t.id = p.topping_id
     WHERE p.key = 'url' AND t.type = 'link'
       ${t?`AND p.topping_id IN (${e.map(()=>`?`).join(`,`)})`:``}`,t?e:[]),r=new Set(e),i=new Map;for(let e of n)!e.value_text||!r.has(e.topping_id)||i.set(e.topping_id,e.value_text);return i}var nt=new Set([`queued`,`active`,`done`,`dropped`]);async function rt(e){if(e.length===0)return new Map;let t=e.length<=900,n=await O.db.exec(`SELECT i.topping_id, i.set_id, s.name AS set_name, s.labels, i.slot, i.rating
     FROM private_entity_effective_marks i
     JOIN status_sets s ON s.id = i.set_id
     WHERE (i.slot IS NOT NULL OR i.rating IS NOT NULL)
       ${t?`AND i.topping_id IN (${e.map(()=>`?`).join(`,`)})`:``}
     ORDER BY i.updated_at DESC, i.set_id`,t?e:[]),r=new Set(e),i=new Map;for(let e of n){if(!r.has(e.topping_id))continue;let t=e.slot&&nt.has(e.slot)?e.slot:null,n=it(e.labels),a={setId:e.set_id,setName:e.set_name,slot:t,statusLabel:t?n[t]??t:null,rating:e.rating},o=i.get(e.topping_id)??[];o.push(a),i.set(e.topping_id,o)}return i}function it(e){try{let t=JSON.parse(e),n={};for(let e of nt)typeof t[e]==`string`&&(n[e]=t[e]);return n}catch{return{}}}function at(t){if(!t)return null;switch(t.kind){case`number`:return t.value;case`money`:return t.amount;case`duration`:return t.seconds;case`checkbox`:return+!!t.value;case`date`:return Date.parse(t.iso)||0;default:return e(t).toLowerCase()}}async function ot(t,n,r){let i=n.key,a=new Map;if(!i.startsWith(`$`)){let e=new Set(r.map(e=>e.id)),t=[];if(r.length>0&&r.length<=2e3){let n=[...e];for(let e=0;e<n.length;e+=500){let r=n.slice(e,e+500);t.push(...await O.db.exec(`SELECT p.topping_id, p.kind, p.value_text, p.value_num, p.value_aux
           FROM properties p
         ...
         FROM properties p JOIN toppings t ON t.id = p.topping_id
         WHERE t.deleted_at IS NULL AND p.key = ?`,[i]));for(let n of t){if(!e.has(n.topping_id))continue;let t=s(n.kind,n.value_text,n.value_num,n.value_aux);t&&a.set(n.topping_id,t)}}let o=new Map;for(let t of r){let n=a.get(t.id),r=i===`$title`||i===`$basename`?t.title:i===`$name`?t.contentRef?.split(`/`).pop()??null:i===`$path`?t.contentRef??null:i===`$folder`?t.contentRef?.split(`/`).slice(0,-1).join(`/`)??null:i===`$ext`?t.contentRef?.split(`.`).pop()?.toLowerCase()??null:i===`$updated`?t.updatedAt??null:i===`$type`?t.type:null,s=n?e(n):r||`No ${i}`,c=n?at(n):r?i===`$updated`?Date.parse(r):r.toLowerCase():null,l=o.get(s)??{order:c,items:[]};l.items.push(t),o.set(s,l)}let c=n.dir===`desc`?-1:1,l=[...o.entries()].sort(([,e],[,t])=>e.order===null?1:t.order===null?-1:typeof e.order==`number`&&typeof t.order==`number`?(e.order-t.order)*c:String(e.order).localeCompare(String(t.order))*c);return{items:l.flatMap(([,e])=>e.items),groups:l.map(([e,t])=>({label:e,count:t.items.length}))}}async function st(e){let t=await O.db.exec(`SELECT t.type, COUNT(*) AS count
       FROM toppings t
      WHERE t.folder_id = ? AND t.deleted_at IS NULL
        AND ${F(`t`)}
      GROUP BY t.type
      ORDER BY type`,[e]);return Object.fromEntries(t.map(e=>[e.type,e.count]))}async function ct(e){return e.length===0?[]:(await O.db.exec(`SELECT DISTINCT t.content_ref
       FROM properties p JOIN toppings t ON t.id = p.topping_id
      WHERE t.source = 'vault' AND t.deleted_at IS NULL AND t.content_ref IS NOT NULL
        AND p.key IN (${e.map(()=>`?`).join(`,`)})`,e)).map(e=>e.content_ref)}async function lt(e){return(await O.db.exec(`SELECT value_text FROM properties WHERE topping_id = ? AND key = 'url'`,[e]))[0]?.value_text??null}async function ut(e){let t=e?`AND t.folder_id = ?`:``;return dt(await O.db.exec(`SELECT p.key, p.kind, COUNT(*) AS item_count
       FROM properties p JOIN toppings t ON t.id = p.topping_id
      WHERE t.deleted_at IS NULL AND ${F(`t`)} ${t}
      GROUP BY p.key, p.kind
      ORDER BY p.key, item_count DESC, p.kind`,e?[e]:[])).map(({key:e,kind:t})=>({key:e,kind:t}))}function dt(e){let t=new Map;for(let n of e){let e=t.get(n.key);(!e||n.item_count>e.item_count||n.item_count===e.item_count&&n.kind.localeCompare(e.kind)<0)&&t.set(n.key,n)}return[...t.values()].sort((e,t)=>e.key.localeCompare(t.key)).map(({key:e,kind:t,item_count:n})=>({key:e,kind:t,itemCount:n}))}async function ft(e){let t=e===null?``:`AND t.folder_id = ?`,n=e===null?[]:[e],[r,i]=await Promise.all([O.db.exec(`SELECT COUNT(*) AS live_item_count
         FROM toppings t
        WHERE t.deleted_at IS NULL
          AND ${F(`t`)} ${t}`,n),O.db.exec(`SELECT p.key, p.kind, COUNT(DISTINCT t.id) AS item_count
         FROM properties p
         JOIN toppings t ON t.id = p.topping_id
        WHERE t.deleted_at IS NULL
          AND ${F(`t`)} ${t}
        GROUP BY p.key, p.kind
        ORDER BY p.key, item_count DESC, p.kind`,n)]);return{liveItemCount:r[0]?.live_item_count??0,fields:dt(i)}}async function pt(e){let t=e?`AND t.folder_id = ?`:``,n=await O.db.exec(`SELECT p.key,
       SUM(CASE
     ...
     GROUP BY p.key ORDER BY p.key`,e?[e]:[]),r=typeof Intl.supportedValuesOf==`function`?new Set(Intl.supportedValuesOf(`currency`)):null;return n.filter(e=>e.invalid>0||!e.codes?!1:e.codes.split(`,`).every(e=>{if(r)return r.has(e);try{return new Intl.NumberFormat(`en`,{style:`currency`,currency:e}),!0}catch{return!1}})).map(e=>e.key)}async function mt(e,t={}){if(t.itemIds?.length===0)return new Map;let n=[],r=[`t.deleted_at IS NULL`,F(`t`)];e&&(r.push(`t.folder_id = ?`),n.push(e)),t.kinds?.length&&(r.push(`p.kind IN (${t.kinds.map(()=>`?`).join(`,`)})`),n.push(...t.kinds)),t.itemIds!==void 0&&t.itemIds.length<=900&&(r.push(`p.topping_id IN (${t.itemIds.map(()=>`?`).join(`,`)})`),n.push(...t.itemIds));let i=await O.db.exec(`SELECT p.topping_id, p.key, p.kind, p.value_text, p.value_num, p.value_aux
     FROM properties p JOIN toppings t ON t.id = p.topping_id
     WHERE ${r.join(` AND `)}`,n),a=t.itemIds?new Set(t.itemIds):null,o=new Map;for(let e of i){if(a&&!a.has(e.topping_id))continue;let t=s(e.kind,e.value_text,e.value_num,e.value_aux);if(!t)continue;let n=o.get(e.topping_id);n||o.set(e.topping_id,n={}),n[e.key]=t}return o}async function ht(e,t){let n=await O.db.exec(`SELECT t.id, t.title, t.content_ref, p.key, p.kind, p.value_text, p.value_num, p.value_aux
       FROM toppings t JOIN properties p ON p.topping_id = t.id AND p.key IN (?, ?)
      WHERE t.source = 'vault' AND t.deleted_at IS NULL AND t.type = 'note'`,[e,t]),r=new Map;for(let e of n){let t=s(e.kind,e.value_text,e.value_num,e.value_aux);if(!t)continue;let n=r.get(e.id);n||r.set(e.id,n={id:e.id,title:e.title,contentRef:e.content_ref,props:{}}),n.props[e.key]=t}return[...r.values()]}async function gt(){return(await O.db.exec(`SELECT COUNT(*) AS count
       FROM content_documents
      WHERE status = 'pending'`))[0]?.count??0}async function _t(e){let t=(await O.db.exec(`SELECT status, media_type, page_count, detail
       FROM content_documents
      WHERE topping_id = ?`,[e]))[0];return t?{status:t.status,mediaType:t.media_type,pageCount:t.page_count,detail:t.detail}:null}function vt(e){let t=e.replace(/[\u0000-\u001f\u007f]/g,` `).split(/\s+/).filter(Boolean);return t.length===0?null:t.map(e=>`"${e.replaceAll(`"`,`""`)}"*`).join(` `)}function $(e,t){for(let n of e){if(n.id===t)return n;let e=$(n.children,t);if(e)return e}return null}function yt(e,t){let n=$(e,t);if(!n)return[t];let r=[],i=e=>{r.push(e.id),e.children.forEach(i)};return i(n),r}async function bt(e,t){let n=vt(e);if(n===null||t!==null&&t.length===0)return{results:[],truncated:!1};let r=[],i=``;t!==null&&(i=`AND t.folder_id IN (${t.map(()=>`?`).join(`,`)})`,r.push(...t));let a=[n,...r,n,...r],o=await O.db.exec(`WITH topping_matches AS (
     ...
     SELECT *
      ...
      LIMIT 101`,a),s=o.length>100;return{results:o.slice(0,100).map(e=>({resultKey:e.result_key,id:e.id,type:e.type,title:e.title,titleMarked:e.title_marked,snippet:e.body_snippet,contentRef:e.source===`vault`?e.content_ref:null,folderId:e.folder_id,folderName:e.folder_name,anchor:e.anchor_page===null?null:{kind:`page`,page:e.anchor_page}})),truncated:s}}async function xt(e){if(e===null)return``;let t=await O.db.exec(`SELECT path, home FROM folders WHERE id = ?`,[e]);return t[0]?we(t[0]):null}export{_ as $,Ye as A,Ce as B,et as C,Ve as D,Me as E,Te as F,h as G,E as H,be as I,p as J,re as K,ye as L,Ue as M,xt as N,Ge as O,Je as P,oe as Q,xe as R,ht as S,$e as T,D as U,Se as V,ie as W,ne as X,te as Y,m as Z,ct as _,Ke as a,b as at,ut as b,Re as c,st as d,de as et,Qe as f,lt as g,Xe as h,He as i,ce as it,bt as j,We as k,_t as l,ot as m,ze as n,se as nt,$ as o,v as ot,I as p,ee as q,Be as r,ue as rt,J as s,yt as t,y as tt,pt as u,gt as v,X as w,mt as x,ft as y,O as z};